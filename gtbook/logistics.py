...
# %% ../logistics.ipynb #bde494e4
import gtsam
import math
import numpy as np

# %% ../logistics.ipynb #27c853be
def gaussian(x: np.ndarray, mean=None, cov=None):
    """Evaluate a 2D Gaussian at x, defaulting to zero mean and identity covariance."""
    if mean is None:
        mean = np.zeros(2)
    if cov is None:
        cov = np.eye(2)
