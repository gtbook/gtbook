...
# %% ../drone.ipynb #96cb1f25
ChebyshevFactor = gtsam.VectorEvaluationFactorChebyshev2
...
class SmoothTrajectory:
    ...
    def fit(path: np.array, N:int=5, a:float=-1, b:float=1, boundaries:bool = False):
        ...
        for k, xyz in enumerate(path):
            ...
            graph.add(ChebyshevFactor(key, xyz, model, 3, N, t_k, a, b))
        
        # If asked, constrain start and end points to be constant velocity:
        if boundaries:
            dt = t[1] - t[0]
            v = (path[1] - path[0])/dt
            graph.add(ChebyshevFactor(key, path[0] - v*dt, constrained3, 3, N, t[0]-dt, a, b))
            graph.add(ChebyshevFactor(key, path[0] - v*2*dt, constrained3, 3, N, t[0]-2*dt, a, b))
            v = (path[-1] - path[-2])/dt
            graph.add(ChebyshevFactor(key, path[-1] + v*dt, constrained3, 3, N, t[-1]+dt, a, b))
            graph.add(ChebyshevFactor(key, path[-1] + v*2*dt, constrained3, 3, N, t[-1]+2*dt, a, b))
        
        # Optimize
        initial = gtsam.Values()
        initial.insert(key, np.zeros((3, N)))
        optimizer = gtsam.LevenbergMarquardtOptimizer(graph, initial)
        result = optimizer.optimize()
        return result.atMatrix(key).T
