...
- Use the root `environment.yml` for development and CI:
  `conda env create -f environment.yml`, then `conda activate nbdev2`.
- Run `pip install -e '.[dev]'` to install gtbook and the nbdev tools.
  Package and development dependencies are maintained in `settings.ini`.
  CI uses the same setup.
